...
COPY . /smarthome
RUN apt-get update
RUN apt install -qy --no-install-recommends build-essential ca-certificates cmake libconfig++-dev libmicrohttpd-dev libssl-dev git
...
COPY --from=build_env smarthome smarthome
